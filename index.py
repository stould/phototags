...
def getphotos(username):
    ...
    response_all = httpGet("/v2.3/%s/photos?access_token=%s&fields=name,link,images" % (user.userid, user.access_token)).decode("utf-8").decode("string_scape")
    response_uploaded = httpGet("/v2.3/%s/photos?access_token=%s&fields=name,link,images&type=uploaded" % (user.userid, user.access_token)).decode("utf-8").decode("string_scape")
...
def getparsedphotos(username):
    ...
    response_all = httpGet("/v2.3/%s/photos?access_token=%s" % (user.userid, user.access_token)).decode("utf-8").decode("string_scape")
    response_photos = httpGet("/v2.3/%s/photos?access_token=%s&type=uploaded" % (user.userid, user.access_token)).decode("utf-8").decode("string_scape")
